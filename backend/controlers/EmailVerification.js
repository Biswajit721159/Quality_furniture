...
let { register, loginUser } = require("../controlers/authUserControllers.js");
...
let LoginVerification = async (req, res) => {
	try {
		...
		return res
			.status(500)
			.json(new ApiResponse(500, null, "Some Error is Found Please Try Again Later"));
	}
};
	...
	try {
		...
		if (user) {
			...
			if (result.modifiedCount === 1) {
				...
				return res
					.status(200)
					.json(new ApiResponse(200, null, `No user found with email: ${email}`));
			}
		} else {
...
let VerifyOTP = async (req, res) => {
	try {
			...
			if (user) {
				...
				if (isOTPCorrect) {
					...
					if (data) {
						...
						if (!finduseronregister) {
							return register(req, res);
						} else {
							return res.status(404).json({ message: "Email Id already exit" });
						}
					} else {
		...
		return res.status(500).json({ message: "Some Error is Found Please Try Again Later" });
	}
};
